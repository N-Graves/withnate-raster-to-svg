...
const JS = new URL("../dist/raster-to-svg.js", import.meta.url);
const CSS = new URL("../dist/raster-to-svg.css", import.meta.url);
...
const workerJs = await readFile(new URL("../dist/raster-to-svg.worker.js", import.meta.url), "utf8");
...
for (const api of network) {
  check(!workerJs.includes(api), `worker makes no network call (${api})`);
}
check(
  (workerJs.match(/fetch\(/g) ?? []).length === 1,
  "worker fetches exactly once - the local wasm and nothing else",
);
...
check(workerUrls.length === 0, "worker references no external URL", workerUrls.join(" "));
